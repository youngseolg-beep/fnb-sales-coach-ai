...
function buildStoreNameMap(stores: StoreRow[]) {
  const map = new Map<number, string>();
  for (const store of stores) {
    map.set(store.id, store.store_name || `Store ${store.id}`);
  }
  return map;
}
...
function aggregateRows(rows: SalesDailyRow[], storeNameMap: Map<number, string>) {
...
export async function loadMasterDashboard(range: MasterDateRange): Promise<MasterDashboardResult> {
  ...
  const storeNameMap = buildStoreNameMap(stores);
  const ranking = aggregateRows(currentRows, storeNameMap);
  const previousRanking = aggregateRows(previousRows, storeNameMap);
