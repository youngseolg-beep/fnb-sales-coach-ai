...
- 메뉴 원가: ${currency} ${Math.round(context.foodCost.directMenuCost).toLocaleString()}
...
- 실질 원가율: ${context.foodCost.foodCostRate.toFixed(1)}%
...
- 별도 목표나 벤치마크가 제공되지 않았으므로 실질 원가율을 높음·낮음 또는 업계·목표 대비로 단정하지 않는다.
