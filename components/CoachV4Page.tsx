...
const CoachV4Page: React.FC<Props> = ({
  ...
  return <main className="mx-auto w-full max-w-[430px] space-y-4 pb-32 text-[#1f1f1f]">
    ...
    <section className="space-y-2">{rows.map((row) => <React.Fragment key={row.key}><button ref={row.key === "report" ? reportActionRef : undefined} type="button" onClick={() => { setOpenPanel((current) => current === row.key ? null : row.key); if (row.key === "report" && !reportMatchesScope && canGenerateReport && !loading && reportStatus !== "generating") void onGenerateReport(); }} className="flex w-full items-center gap-3 rounded-[13px] border border-[#eee8e3] bg-white p-3 text-left shadow-[0_2px_8px_rgba(70,54,42,0.03)]"><span className={`flex h-9 w-9 items-center justify-center rounded-[10px] bg-[#faf7ff] ${row.color}`}><i className={`fa-solid ${row.icon}`} /></span><span className="min-w-0 flex-1"><b className="block text-[13px]">{row.title}</b><small className="mt-0.5 block truncate text-[10px] text-[#736a63]">{row.copy}</small></span><span className="rounded-[8px] border border-[#ede7df] px-2 py-1 text-[10px] font-semibold text-[#7a604c]">{(loading || reportStatus === "generating") && row.key === "report" ? "분석 중" : row.badge}</span><i className="fa-solid fa-chevron-right text-[10px] text-[#7f746d]" /></button>{openPanel === row.key && <div className="overflow-hidden rounded-[13px] border border-[#eee8e3] bg-white p-3"><CoachPeriodSelector activePeriod={activePeriod} periodRange={periodRange} allowToday={row.key !== "engineering"} onPeriodChange={onPeriodChange} onCustomRangeChange={onCustomRangeChange} />{row.key === "analysis" ? <PeriodAnalysis periodRange={periodRange} comparisonRange={comparisonRange} salesChangeRate={salesChangeRate} ordersChangeRate={ordersChangeRate} visitorsChangeRate={visitorsChangeRate} aovChangeRate={aovChangeRate} conversion={conversion} trendRows={trendRows} highestSales={highestSales} currentMenus={currentPeriodMenus} comparisonMenus={comparisonPeriodMenus} country={country} /> : row.key === "report" ? <V4Report report={report} reportMatchesScope={reportMatchesScope} loading={loading} persistedStatus={reportStatus} error={reportError} canGenerateReport={canGenerateReport} onRetry={onGenerateReport} /> : row.key === "engineering" ? engineeringContent : boostContent}</div>}</React.Fragment>)}</section>
  </main>;
};
...
const V4Report: React.FC<{ report: string; reportMatchesScope: boolean; loading: boolean; persistedStatus: "generating" | "completed" | "failed" | null; error: string; canGenerateReport: boolean; onRetry: () => Promise<void>; sectionTitles: string[] }> = ({ report, reportMatchesScope, loading, persistedStatus, error, canGenerateReport, onRetry, sectionTitles }) => {
  if (loading || persistedStatus === "generating") return <div className="rounded-xl bg-[#fbfaff] p-5 text-center"><div className="mx-auto h-7 w-7 animate-spin rounded-full border-2 border-[#e4dcff] border-t-[#7259dd]" /><p className="mt-3 text-sm font-semibold text-[#423856]">AI가 운영 데이터를 분석하고 있습니다...</p>{!loading && <><p className="mt-2 text-[11px] text-[#62587c]">이전 AI 분석이 완료되지 않았습니다.</p><button type="button" onClick={() => void onRetry()} className="mt-3 rounded-lg border border-[#d8bbb0] px-3 py-2 text-xs font-semibold text-[#81513c]">다시 분석</button></>}</div>;
