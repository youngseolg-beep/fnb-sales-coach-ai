...
const SalesV4Page: React.FC<Props> = ({ model, onReset, onSave }) => {
  ...
  return (
    <main className="mx-auto w-full max-w-[430px] space-y-3 pb-36 text-[#1f1f1f] lg:max-w-[1180px] lg:pb-10">
      ...
      <section data-tour="sales-reconciliation" className="rounded-[14px] border border-[#e7dfd9] bg-white px-3.5 py-3 shadow-[0_2px_8px_rgba(70,54,42,0.025)]">
  ...
  return <div className="mt-3 space-y-2.5 border-t border-[#eee8e3] pt-3">
    ...
    {model.ocrRawText && <>
      <div className="grid grid-cols-2 gap-1.5 rounded-[9px] border border-[#e7dfd9] bg-white p-2.5 text-[10px]">
        ...
        <ReviewMetric label="자동 매칭" value={`${model.confirmedItems.length}개`} />
        <ReviewMetric label="확인 필요" value={`${model.needsReviewItems.length}개`} />
        <ReviewMetric label="OCR 계산 합계" value={formatCurrencyValue(model.scanTotal, country)} />
      </div>
      <p className="px-1 text-[10px] text-[#766c66]">{currencySummary} · {canApply ? "적용 가능" : model.ocrApplyBlockReason || "확인 필요"}</p>
      ...
      <button type="button" disabled={model.isOcrApplyBlocked} onClick={model.applyOcr} className="h-9 w-full rounded-[8px] bg-[#8b5e3c] px-4 text-[11px] font-semibold text-white disabled:cursor-not-allowed disabled:bg-[#d8d1cb]">{model.isOcrApplyBlocked ? model.ocrApplyBlockReason || "메뉴 합계 또는 통화 확인 후 적용 가능" : "데이터 입력창에 적용하기"}</button>
    </>}
  </div>;
};
