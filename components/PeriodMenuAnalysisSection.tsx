    ...
    <section className="space-y-3 md:space-y-4">
      <section className="overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-sm md:rounded-[28px]">
        <div className="border-b border-slate-100 px-3 py-3 md:px-6 md:py-4">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
            <div>
              <div className="inline-flex rounded-full bg-indigo-50 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-indigo-500 md:text-[11px] md:tracking-[0.18em]">
                Period Analysis
              </div>
              <h3 className="mt-2 text-base font-black tracking-tight text-slate-900 md:text-2xl">
                기간 분석
              </h3>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input
                ...
                className="h-10 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-black text-slate-700 outline-none transition-all focus:border-indigo-300 focus:bg-white focus:ring-2 focus:ring-indigo-100 md:h-11 md:rounded-2xl md:px-4 md:text-sm"
              />
              <span className="hidden text-xs font-black text-slate-300 sm:block md:text-sm">~</span>
              <input
                type="date"
              ...
              <button
                ...
                className="h-10 rounded-xl bg-indigo-500 px-4 text-xs font-black text-white transition-all hover:bg-indigo-600 disabled:bg-slate-300 md:h-11 md:rounded-2xl md:px-5 md:text-sm"
              >
        ...
        <div className="space-y-3 bg-slate-50/60 px-3 py-3 md:space-y-4 md:px-6 md:py-5">
          ...
          <div className="grid grid-cols-2 gap-2.5 md:gap-3 xl:grid-cols-4">
            {summaryCards.map((card) => (
              <div
                key={card.label}
                className="rounded-[18px] border border-slate-200 bg-white px-3 py-3 shadow-sm md:rounded-[22px] md:px-4 md:py-4"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="text-xs font-black text-slate-500 md:text-sm">{card.label}</div>
                  <div className={`rounded-full px-2 py-0.5 text-[10px] font-black md:px-2.5 md:py-1 md:text-[11px] ${rateTone(card.rate)}`}>
                    {card.rate >= 0 ? "+" : ""}
                    {card.rate.toFixed(1)}%
                  </div>
                </div>

                <div className="mt-2 text-lg font-black tracking-tight text-slate-900 md:mt-3 md:text-2xl">
                  {card.current}
                </div>

                <div className="mt-1.5 text-[10px] font-black uppercase tracking-[0.08em] text-slate-400 md:mt-2 md:text-[11px] md:tracking-[0.12em]">
                  Compare
                </div>
                <div className="mt-0.5 text-xs font-bold text-slate-500 md:mt-1 md:text-sm">
                  {card.compare}
                </div>

                <div className="mt-2.5 text-[10px] font-black text-slate-500 md:mt-3 md:text-xs">
                  {rateText(card.rate)}
                </div>
              </div>
            ))}
      ...
      <section className="overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-sm md:rounded-[28px]">
        ...
        <div className="max-h-96 overflow-auto">
          <table className="w-full min-w-[420px] table-fixed text-xs md:min-w-[560px] md:text-sm">
            <colgroup>
              <col className="w-[92px] md:w-[120px]" />
              <col className="w-[110px] md:w-[140px]" />
              <col className="w-[72px] md:w-[90px]" />
              <col className="w-[78px] md:w-[100px]" />
            </colgroup>
            <thead className="sticky top-0 bg-white">
              <tr className="border-b border-slate-100">
                <th className="px-2 py-2.5 text-left text-[10px] font-black uppercase tracking-[0.08em] text-slate-400 md:px-4 md:py-3 md:text-[11px] md:tracking-[0.12em]">
                  날짜
                </th>
                <th className="px-2 py-2.5 text-right text-[10px] font-black uppercase tracking-[0.08em] text-slate-400 md:px-4 md:py-3 md:text-[11px] md:tracking-[0.12em]">
                  매출
                </th>
                <th className="px-2 py-2.5 text-right text-[10px] font-black uppercase tracking-[0.08em] text-slate-400 md:px-4 md:py-3 md:text-[11px] md:tracking-[0.12em]">
                  주문
              ...
              {(periodStats?.list || []).length > 0 ? (
                (periodStats?.list || []).map((row: any) => (
                  <tr key={row.date} className="border-b border-slate-100 last:border-b-0">
                    <td className="truncate px-2 py-2.5 font-bold text-slate-700 md:px-4 md:py-3">
                      {row.date}
                    </td>
                    <td className="px-2 py-2.5 text-right font-black text-slate-900 md:px-4 md:py-3">
                      ${Number(row.total_sales || 0).toLocaleString()}
                    </td>
                    <td className="px-2 py-2.5 text-right font-medium text-slate-600 md:px-4 md:py-3">
