...
interface ReportDisplayProps {
  ...
  sortedMenuEngineering: any; // 가능하면 나중에 타입 정리
  boostPlans: any[]; // 가능하면 나중에 타입 정리
}
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-indigo-50 overflow-hidden mb-12">
      ...
      <div className="p-6 space-y-6">
          ...
          menuEngineeringResult && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 mt-6 overflow-hidden">
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-4 bg-slate-50 rounded-t-xl border-b border-slate-200"
                type="button"
              >
                ...
                <i
                  className={`fa-solid ${
                    isMeSummaryOpen ? "fa-chevron-up" : "fa-chevron-down"
                  ...
                  {sortedMenuEngineering && (
                    <div className="bg-white rounded-xl shadow-sm border border-slate-100">
                      <p className="text-xs text-slate-500 p-4 border-b border-slate-100">
                        ※ 선택한 날짜 기준 최근 7일 / 인기도 기준: 최근 7일 평균 판매수량{" "}
                        {sortedMenuEngineering.popularityThreshold}개 / 수익성 기준: 평균 1개당
                        이익(판매가-원가) ${sortedMenuEngineering.profitabilityThreshold}
                      </p>

                      <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Stars */}
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                          <h4 className="font-black text-slate-900 text-md mb-2">
                            Stars (고인기, 고수익)
                          </h4>
                          <ul className="space-y-1 text-xs text-slate-700">
                            {sortedMenuEngineering.starsTop3?.length > 0 ? (
                              sortedMenuEngineering.starsTop3.map((item: any, index: number) => (
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                          ...
                          <ul className="space-y-1 text-xs text-slate-700">
                            {sortedMenuEngineering.cashCowsTop3?.length > 0 ? (
                              sortedMenuEngineering.cashCowsTop3.map((item: any, index: number) => (
                                <li key={index}>{item}</li>
                              ))
                            ) : (
                              <li>없음</li>
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                          ...
                          <ul className="space-y-1 text-xs text-slate-700">
                            {sortedMenuEngineering.puzzlesTop3?.length > 0 ? (
                              sortedMenuEngineering.puzzlesTop3.map((item: any, index: number) => (
                                <li key={index}>{item}</li>
                              ))
                            ) : (
                              <li>없음</li>
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                          ...
                          <ul className="space-y-1 text-xs text-slate-700">
                            {sortedMenuEngineering.dogsTop3?.length > 0 ? (
                              sortedMenuEngineering.dogsTop3.map((item: any, index: number) => (
                                <li key={index}>{item}</li>
                              ))
                            ) : (
                              <li>없음</li>
        ...
        {/* ─────────────────────────────────────────────
            Boost Plans Section (7일 이상 + 플랜 있을 때)
           ───────────────────────────────────────────── */}
        {menuEngineeringResult &&
          menuEngineeringResult.analyzedDatesCount >= 7 &&
          boostPlans.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 mt-4">
              <div className="bg-slate-50 border-b border-slate-100 p-4">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Puzzles (저인기, 고수익) 메뉴를 위한 추천 세트 구성 및 일일 목표입니다.
                </p>
              </div>
              <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                {boostPlans.map((plan: any, index: number) => (
                  <div
                    key={index}
                    className="bg-indigo-50 p-4 rounded-lg border border-indigo-100 space-y-2"
                  >
                    ...
                    <p className="text-xs text-indigo-700">
                      <strong>구성:</strong> {plan.setComposition}
                    </p>
                    <p className="text-xs text-indigo-700">
                      <strong>할인:</strong> {plan.discount}
                    </p>
                    <p className="text-xs text-indigo-700">
                      <strong>하루 목표 수량:</strong> {plan.dailyTargetQty}개
                    </p>
                    {plan.reason && (
                      <p className="text-xs text-indigo-700">
