    ...
    <div className="w-full pb-4">
      <div className="mb-4 grid grid-cols-3 gap-1.5 sm:mb-5 sm:flex sm:flex-wrap sm:gap-2">
        <button
          ...
          className={`min-h-11 whitespace-nowrap rounded-xl border px-1 py-2 text-[12px] font-semibold leading-none transition sm:px-4 sm:text-sm ${
            activeTab === "pending"
      ...
      <div className="space-y-4">
        {filteredList.length === 0 ? (
          <div className="rounded-[20px] border border-[#ECE7E1] bg-white px-4 py-7 text-center text-[12px] text-[#9C948E] sm:py-10 sm:text-sm">
          ...
          filteredList.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="rounded-[20px] border border-[#ECE7E1] bg-white p-4 sm:p-6"
              >
                {isEditing ? (
                  <div className="space-y-4 sm:space-y-5">
                    <div>
                      <p className="text-[13px] font-bold text-[#1F1F1F] sm:text-sm">신청 정보 수정</p>
                      <p className="mt-1 text-[11px] text-[#9C948E] sm:text-xs">변경한 신청 정보는 저장 후 반영됩니다.</p>
                    </div>
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
                      <label className="text-[11px] font-semibold text-[#706A66] sm:text-xs">
                        점주 성함
                        <input
                          value={editForm.owner_name}
                          onChange={(e) => handleEditChange("owner_name", e.target.value)}
                          className="mt-1 h-11 w-full rounded-[14px] border border-[#ECE7E1] bg-[#FFFDFC] px-3 text-[13px] text-[#1F1F1F] outline-none focus:border-[#A8866B] sm:mt-1.5 sm:h-12 sm:text-sm"
                        />
                      </label>
                      <label className="text-[11px] font-semibold text-[#706A66] sm:text-xs">
                        연락처
                      ...
                      <label className="text-[11px] font-semibold text-[#706A66] sm:col-span-2 sm:text-xs">
                        이메일
                        <input
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:flex sm:justify-end">
                      <button
                        ...
                        disabled={loading}
                        className="min-h-11 rounded-xl bg-[#8B6F5B] px-4 text-[13px] font-bold text-white transition hover:bg-[#765C49] disabled:cursor-not-allowed disabled:opacity-50 sm:px-5 sm:text-sm"
                      >
                        저장
                      </button>
                      <button
                        onClick={cancelEdit}
                        disabled={loading}
                        className="min-h-11 rounded-xl border border-[#ECE7E1] bg-white px-4 text-[13px] font-bold text-[#706A66] transition hover:bg-[#F7F2EE] disabled:cursor-not-allowed disabled:opacity-50 sm:px-5 sm:text-sm"
                      >
                        취소
                      </button>
                    </div>
                  ...
                  <div>
                    <div className="flex flex-col gap-1.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                      <div className="min-w-0">
                        <h3 className="break-words text-[15px] font-bold text-[#1F1F1F] sm:text-lg">{item.store_name}</h3>
                        <p className="mt-0.5 text-[12px] text-[#706A66] sm:mt-1 sm:text-sm">{getBrandLabel(item.brand)}</p>
                      </div>
                      <span className={`inline-flex w-fit rounded-full px-2 py-0.5 text-[10.5px] font-bold sm:px-2.5 sm:py-1 sm:text-xs ${STATUS_STYLES[status] || "bg-[#F4F1EE] text-[#706A66]"}`}>
                        {STATUS_LABELS[status] || item.status || "-"}
                      </span>
                    </div>

                    <dl className="mt-3 grid grid-cols-1 gap-x-6 gap-y-2 text-[13px] sm:mt-5 sm:grid-cols-2 sm:gap-y-4 sm:text-sm lg:grid-cols-4">
                      <div className="min-w-0">
                        <dt className="text-[10.5px] font-semibold text-[#9C948E] sm:text-xs">점주</dt>
                        <dd className="mt-0.5 break-words font-medium text-[#1F1F1F] sm:mt-1">{item.owner_name || "-"}</dd>
                      </div>
                      <div className="min-w-0">
                        <dt className="text-[10.5px] font-semibold text-[#9C948E] sm:text-xs">연락처</dt>
                        <dd className="mt-0.5 break-words font-medium text-[#1F1F1F] sm:mt-1">{item.phone || "-"}</dd>
                      </div>
                      <div className="min-w-0">
                        <dt className="text-[10.5px] font-semibold text-[#9C948E] sm:text-xs">이메일</dt>
                        <dd className="mt-0.5 break-all font-medium text-[#1F1F1F] sm:mt-1">{item.email || "-"}</dd>
                      </div>
                      <div className="min-w-0">
                        <dt className="text-[10.5px] font-semibold text-[#9C948E] sm:text-xs">국가</dt>
                        <dd className="mt-0.5 break-words font-medium text-[#1F1F1F] sm:mt-1">{getCountryLabel(item.country)}</dd>
                      </div>
                    </dl>

                    <div className="mt-3 border-t border-[#ECE7E1] pt-3 sm:mt-5 sm:pt-5">
                      {status === "pending" && (
                        <div className="max-w-md space-y-2 sm:space-y-3">
                          <label className="block text-[11px] font-semibold text-[#706A66] sm:text-xs">
                            초기 비밀번호
                            <input
                              type="password"
                              inputMode="numeric"
                              ...
                              placeholder="숫자 6자리"
                              className="mt-1 h-11 w-full rounded-[14px] border border-[#ECE7E1] bg-[#FFFDFC] px-3 text-[13px] text-[#1F1F1F] outline-none placeholder:text-[12px] focus:border-[#A8866B] sm:mt-1.5 sm:h-12 sm:text-sm sm:placeholder:text-[13px]"
                            />
                          </label>
                          <button
                            onClick={() => handleApprove(item)}
                            disabled={loading}
                            className="h-11 w-full rounded-xl bg-[#8B6F5B] px-4 text-[13px] font-semibold text-white transition hover:bg-[#765C49] disabled:cursor-not-allowed disabled:opacity-50 sm:text-sm"
                          >
                            승인
                          </button>
                          <div className="grid grid-cols-2 gap-2">
                            <button
                              ...
                              className="min-h-11 rounded-xl border border-[#ECE7E1] bg-white px-3 text-[13px] font-bold text-[#706A66] transition hover:bg-[#F7F2EE] disabled:cursor-not-allowed disabled:opacity-50 sm:px-4 sm:text-sm"
                            >
                              수정
                            </button>
                            <button
                              ...
                              disabled={loading}
                              className="min-h-11 rounded-xl bg-[#F9EEEE] px-3 text-[13px] font-bold text-[#9A5F5F] transition hover:bg-[#F4E2E2] disabled:cursor-not-allowed disabled:opacity-50 sm:px-4 sm:text-sm"
                            >
                              거절
                            </button>
                          </div>
                          <button
                            ...
                            disabled={loading || isPasswordUpdating}
                            className="min-h-11 w-full rounded-xl px-3 text-[13px] font-semibold text-[#9A5F5F] transition hover:bg-[#FCF5F5] disabled:cursor-not-allowed disabled:opacity-50 sm:px-4 sm:text-sm"
                          >
                            삭제
                          </button>
                        </div>
                        ...
                        <div className="max-w-md space-y-2 sm:space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <button
                              onClick={() => handleDelete(item)}
                              disabled={loading || isPasswordUpdating}
                              className="min-h-11 rounded-xl px-3 text-[13px] font-semibold text-[#9A5F5F] transition hover:bg-[#FCF5F5] disabled:cursor-not-allowed disabled:opacity-50 sm:px-4 sm:text-sm"
                            >
                              삭제
                            </button>
                          </div>
