...
const PeriodTopMenuCompare: React.FC<PeriodTopMenuCompareProps> = ({
  ...
  const [sortBy, setSortBy] = useState<"qty" | "sales">("qty");
  ...
  }, [currentMenus, comparisonMenus, sortBy]);
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h3 className="text-lg font-bold text-gray-900">Top10 메뉴 기간 비교</h3>
      ...
      {!canShow ? (
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border-separate border-spacing-y-2">
            ...
            <tbody>
              {top10.map((row, idx) => (
                <tr key={row.name} className="rounded-xl bg-gray-50 text-sm text-gray-800">
                  ...
                  <td className="px-3 py-3">{numberFmt.format(row.currentQty)}</td>
                  <td className="px-3 py-3">{numberFmt.format(row.previousQty)}</td>
                  <td className={`px-3 py-3 font-semibold ${getDeltaClass(row.diffQty)}`}>
                    {getDeltaText(row.diffQty)}
                  </td>
                  <td className="px-3 py-3">{currencyFmt.format(row.currentSales)}</td>
                  <td className="px-3 py-3">{currencyFmt.format(row.previousSales)}</td>
                  <td className={`rounded-r-xl px-3 py-3 font-semibold ${getDeltaClass(row.diffSales)}`}>
